...
> **📘 MVP Stack**: This guide follows the MVP migration plan using PostgreSQL (transactional + JSONB catalog + FTS), Redis (cache), and Outbox pattern (async events). See `wiki/MVP_STACK_MIGRATION.md` for details.
...
### 4. Setup Infrastructure (MVP Stack)
...
```powershell
# PostgreSQL migrations (includes FTS + Outbox table)
cd ..\scripts
